...
function PaginationLink({ children, enabled, href }) {
  if (!enabled) {
    return (
      <span className="border rounded text-slate-500 text-sm cursor-not-allowed">
        {children}
      </span>
    );
  }
  return (
    <Link
      href={href}
      className="border rounded text-slate-300 text-sm hover:bg-orange-300 hover:text-slate-700"
    >
      {children}
    </Link>
  );
